...
def controle_veiculos():
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cursor:
                # Query com LEFT JOIN para buscar o nome do técnico
                # A ordem das colunas no SELECT DEVE CORRESPONDER EXATAMENTE à ordem esperada pelo HTML:
                # id, placa, tecnico_nome, tipo_registro, data, km, litros, valor, proxima_troca_km, proxima_troca_data, observacoes
                query = """
                    SELECT 
                        cv.id, 
                        cv.placa, 
                        u.usuario AS tecnico_nome, -- Nome do técnico vindo da tabela 'usuarios'
                        cv.tipo_registro, 
                        cv.data, 
                        cv.km, 
                        cv.litros, 
                        ...
                        cv.observacoes,
                        cv.usuario_id -- Adicionado para controle de permissão de exclusão no front-end (opcional)
                ...
                # NOVO: Restrição para técnicos na exibição dos registros
                if user_type == 'tecnico':
                    conditions.append("cv.tipo_registro = 'abastecimento'")
                    conditions.append("cv.usuario_id = %s")
                    params.append(user_id_logado)
                    # Força o filtro de técnico para o próprio técnico logado
                    filtro_tecnico = session.get('nome') or session.get('usuario') # Para o valor do input HTML
                else: # Admin - aplica todos os filtros
                    if filtro_placa:
                        conditions.append("cv.placa ILIKE %s")
                    ...
                    # O filtro de técnico para admin usa o u.usuario da tabela de JOIN
                    if filtro_tecnico:
                ...
                if registros_db:
                    ...
                    # Converte os resultados em uma lista de listas para facilitar a adição do consumo
                    registros_para_processar = [list(reg) for reg in registros_db]

                    # Adiciona um placeholder para o consumo (r[11]) em cada registro
                    # E adiciona o usuario_id para o front-end (r[12])
                    for reg_list in registros_para_processar:
                        reg_list.append('-') # Este será o r[11] para consumo
                        # O user_id que foi buscado no select está no índice 11, mas o HTML não o usa diretamente
                        # Para manter a ordem, vamos passá-lo e o HTML pode ignorar
                        # Mas se vc quiser que o HTML saiba o user_id para decidir botões de editar/excluir,
                        # ele precisará ser passado.
                        # Na query SELECT, adicionei 'cv.usuario_id'. Ele virá no reg_list[11]
                        # O consumo virá no reg_list[12] após a adição do placeholder.
                        # Ajustei o código do consumo abaixo para r[12]
                        
                        # Correção para o índice do usuario_id e consumo
                        # Originalmente: (id, placa, tecnico_nome, tipo, data, km, litros, valor, proxima_km, proxima_data, obs, usuario_id)
                        # Consumo é inserido no final.
                        # Então, r[11] é o usuario_id, r[12] será o consumo.
                        pass # usuario_id já está vindo do banco. Consumo será adicionado.

                    # Mapeia por ID para facilitar a atualização do consumo
                    ...
                    """)
                    abastecimentos_ordenados = cursor.fetchall()

                    ultimo_km_por_placa = {}
                    for ab in abastecimentos_ordenados:
                        # Abastecimentos ordenados: (id, placa, data, km, litros)
                        ab_id, ab_placa, ab_data, ab_km, ab_litros = ab

                        if ab_placa in ultimo_km_por_placa:
                            km_anterior = ultimo_km_por_placa[ab_placa]['km']
                            litros_anterior = ultimo_km_por_placa[ab_placa]['litros']
                            
                            if litros_anterior is not None and ab_litros is not None and ab_km > km_anterior:
                                distancia_percorrida = ab_km - km_anterior
                                if litros_anterior > 0:
                                    consumo_km_litro = distancia_percorrida / litros_anterior
                                    
                                    # Atualiza o consumo no registro correspondente (agora no índice 12)
                                    if ab_id in registros_por_id:
                                        registros_por_id[ab_id].append(f'{consumo_km_litro:.2f} Km/L') # Adiciona no final
                    ...
                    # Converte os registros formatados de volta para a lista final para o template
                    registros_com_consumo = list(registros_por_id.values())

                    # Formata os campos KM e Próxima Troca KM
    ...
    return render_template('controle_veiculos.html',
                            ...
                            user_type=user_type, # Passa o tipo de usuário para o HTML
                            user_id_logado=user_id_logado) # Passa o ID do usuário para o HTML
