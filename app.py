...
BLING_REDIRECT_URI = os.environ.get('BLING_REDIRECT_URI', 'https://5dff04c6d985.ngrok-free.app/callback') 
...
def menu():
    ...
    if tipo_usuario == 'admin':
        ...
        print("DEBUG: Alertas de Geradores Encontrados:", alertas_geradores)
    ...
    return render_template('menu.html',
                            ...
                            alertas_geradores=alertas_geradores)
...
def buscar_alertas_manutencao_geradores():
    """
    Busca no BD os geradores com manutenção próxima (em 45 dias) ou vencida.
    
    Retorna uma lista de dicionários:
    [{'tag': 'TAG123', 'item': 'Próx. Óleo', 'vencimento': '25/12/2025', 'status': 'VENCIDO/PRÓXIMO'}]
    """
    alertas = []
    today = datetime.now().date()
    limite_proximo = today + timedelta(days=45)
    
    # Colunas de data para verificar o vencimento (indices no SELECT abaixo):
    # data_proxima_oleo (4), data_proximo_filtro_diesel (5)
    
    # OBS: O SELECT deve buscar as COLUNAS corretas para o Jinja2 no HTML funcionar
    # no template controle_geradores.html (onde o índice r[10] é data_oleo)
    # Aqui vamos usar um SELECT que traga apenas a TAG e as datas de vencimento que nos interessam
    ...
    WHERE 
        data_proxima_oleo IS NOT NULL OR data_proximo_filtro_diesel IS NOT NULL
    ORDER BY 
        data_manutencao DESC
    LIMIT 100; -- Limite para não carregar muito (considere buscar apenas o último registro por TAG)
    """
    # ***NOTA: É altamente recomendável refazer essa busca para pegar APENAS o registro mais recente para cada gerador (TAG).
    # Como não temos o SQL exato, vamos assumir que o alerta deve ser verificado para TODOS os registros e
    # você fará o ajuste para o "mais recente" em sua rota principal.
    ...
    try:
        ...
        for tag, data_oleo, data_diesel in resultados:
            
            # --- Verifica Próx. Óleo ---
            if data_oleo and isinstance(data_oleo, datetime) and (data_oleo <= limite_proximo):
                status = "VENCIDO" if data_oleo <= today else f"PRÓXIMO ({data_oleo.strftime('%d/%m/%Y')})"
                alertas.append({
                    'tag': tag,
                    'item': 'Troca de Óleo',
                    'vencimento': data_oleo.strftime('%d/%m/%Y'),
                    'status': status
                })

            # --- Verifica Próx. Filtro Diesel ---
            if data_diesel and isinstance(data_diesel, datetime) and (data_diesel <= limite_proximo):
                status = "VENCIDO" if data_diesel <= today else f"PRÓXIMO ({data_diesel.strftime('%d/%m/%Y')})"
                alertas.append({
                    'tag': tag,
                    'item': 'Troca Filtro Diesel',
                    'vencimento': data_diesel.strftime('%d/%m/%Y'),
                    'status': status
                })
                
        # Remove duplicados se um mesmo gerador tiver alertas de óleo e diesel (opcional)
        # e ordena para mostrar os vencidos primeiro
        alertas_ordenados = sorted(alertas, key=lambda x: (x['status'] != 'VENCIDO', x['vencimento']))
        
        return alertas_ordenados
...
def controle_geradores():
    ...
    if request.method == 'POST':
        # ... (Toda a sua lógica de POST: Exclusão, Inserção, Cálculos) ...
        acao = request.form.get('acao') 

        # 1.1 Lógica de Exclusão
        ...
        # 1.2 Lógica de Inserção (salvar_registro)
        elif acao == 'salvar_registro':
            # ... (Toda a sua lógica de obtenção de dados e cálculos de previsão) ...
            ...
            try:
                ...
                # 2. Cálculos de Previsão de Próxima Troca
                data_manutencao = datetime.strptime(data_manutencao_str, '%Y-%m-%d').date()
                DOIS_ANOS = timedelta(days=365 * 2) 
                
                proxima_data_oleo = data_manutencao + timedelta(days=365)
                proximo_horimetro_oleo = horimetro_atual + 250
                
                proxima_data_filtro = data_manutencao + DOIS_ANOS if troca_filtro_ar else None
                proxima_data_liquido = data_manutencao + DOIS_ANOS if troca_liq_arref else None
                proxima_data_correia = data_manutencao + DOIS_ANOS if troca_correia else None 
                proxima_data_baterias = data_manutencao + DOIS_ANOS if troca_baterias else None

                data_proximo_filtro_diesel = None
                if filtro_diesel_periodo == '6meses':
                    ...
                    data_proximo_filtro_diesel = data_manutencao + timedelta(days=365)
                
                # 3. Formatação dos Dados para SQL
                data_manutencao_sql = data_manutencao.strftime('%Y-%m-%d')
                data_oleo_sql = proxima_data_oleo.strftime('%Y-%m-%d')
    ...
    # 2.1 Captura dos Filtros
    # O valor padrão é string vazia para o caso de não haver filtro
    ...
    # 2.2 Construção das Condições de Filtragem
    
    # Filtro 1: Gerador/Cliente (Busca parcial 'ILIKE' para case-insensitive)
    if filtro_gerador:
        conditions.append("gerador ILIKE %s")
        params.append(f'%{filtro_gerador}%')

    # Filtro 2: Data (Busca exata na data_manutencao)
    if filtro_data:
        conditions.append("data_manutencao = %s")
        params.append(filtro_data) 

    # Filtro 3: Status (Busca exata)
    if filtro_status:
    ...
    # 2.3 Execução da Query e Lógica de Alerta (APLICADA AOS REGISTROS FILTRADOS)
    
    # 🚨 CONFIGURAÇÃO DO ALERTA: (Mantida para o caso de alertas na própria tela de controle)
    ALERT_THRESHOLD_DAYS = 45
    today = date.today() 
    maintenance_items = {
        10: "Troca de Óleo (Data)", 
        18: "Filtro Diesel", 
        12: "Filtro de Ar (2 Anos)", 
        13: "Líquido de Arrefecimento (2 Anos)", 
        14: "Correia (2 Anos)", 
        15: "Baterias (2 Anos)"
    }

    try:
        # A conexão é usada aqui. Se o POST falhou, ela já foi fechada.
        # No GET, ela está aberta desde o início.
        cursor = conn.cursor()
        ...
        # --- LÓGICA DE ALERTA (Mantida, rodando sobre os registros FILTRADOS) ---
        for r in registros:
            tag = r[1]
            ...
            for index, item_name in maintenance_items.items():
                ...
                if due_date and (isinstance(due_date, datetime) or isinstance(due_date, date)):
                    ...
                    # Alerta Vencido ( <= 0 dias restantes)
                    if days_left <= 0:
                        alerts.append({
                            "tag": tag,
                            "item": item_name,
                    ...
                    # Alerta Próximo (dentro de 45 dias)
                    elif 0 < days_left <= ALERT_THRESHOLD_DAYS:
                        alerts.append({
                            "tag": tag,
                            "item": item_name,
                            "vencimento": due_date.strftime('%d/%m/%Y'),
                            "status": f"PRÓXIMO ({days_left} dias)"
                        })

    except Exception as e:
        print(f"Erro ao buscar registros ou gerar alertas: {e}")
        flash(f"Erro ao aplicar filtros: {e}", 'error') # Adiciona feedback de erro no front-end
        
    finally:
        # Garante que a conexão seja fechada, independente de ter sido aberta no POST ou usada no GET
        conn.close() 
    
    # 2.4 Renderiza o Template, passando os filtros de volta
    return render_template('controle_geradores.html', 
                            registros=registros, 
                            today=today, 
                            alerts=alerts,
                            # Variáveis para manter o estado do filtro na tela:
