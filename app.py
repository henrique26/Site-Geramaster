...
def menu():
    ...
    # Se for admin, pega alertas da sessão (e já remove da sessão depois de mostrar)
...
def controle_veiculos():
    ...
    if request.method == 'POST':
        ...
        proxima_troca_km = int(request.form['proxima_troca_km']) if request.form.get('proxima_troca_km') else None
        proxima_troca_data = request.form['proxima_troca_data'] or None
        observacoes = request.form.get('observacoes')
        ...
        with get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("""
                    ...
                    (placa, tipo_registro, data, km, litros, valor, proxima_troca_km, proxima_troca_data, observacoes)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (placa, tipo_registro, data_reg, km, litros, valor, proxima_troca_km, proxima_troca_data, observacoes))
                conn.commit()

        flash('Registro salvo com sucesso!', 'success')
        return redirect(url_for('controle_veiculos'))

    # Consulta para mostrar histórico (ordenado pela data)
    with get_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM controle_veiculos ORDER BY data DESC, id DESC")
            registros = cursor.fetchall()

    return render_template('controle_veiculos.html', registros=registros)
