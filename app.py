...
def logout():
    ...
    # O parâmetro logged_out=true avisa o Javascript para apagar o "Lembrar de mim"
...
def rdv():
    ...
    # --- 1. REGISTRO DE NOVO RDV (POST) ---
    if request.method == 'POST' and tipo_usuario in ['tecnico', 'admin']:
        try:
            if request.form.get('acao') != 'excluir_rdv':
                ...
                with get_connection() as conn:
                    with conn.cursor() as cursor:
                        cursor.execute("INSERT INTO rdvs (usuario, valor, descricao, data) VALUES (%s, %s, %s, %s)",
                                       (usuario, valor, descricao, data_obj))
                        conn.commit()
    ...
    try:
        with get_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
                ...
                # Preparar Filtros de Data (Conversão Segura)
                ...
                if filtro_data_ini and filtro_data_fim:
                    try:
                        ...
                        print(f"DEBUG: Filtrando de {d_ini} até {d_fim}")
                    except ValueError: 
                        print("DEBUG: Data inválida recebida")
                ...
                # AQUI ESTAVA O ERRO: O Python só entrava aqui se as datas fossem válidas
                if d_ini and d_fim:
                ...
                # Debug da Query
                # print(cursor.mogrify(q_rdv, tuple(p_rdv))) 
                ...
                if is_admin:
                    ...
                    # Calcula saldo (Geral ou do período?)
                    # Geralmente saldo é acumulado total, independente do filtro de data da tabela
                    for tec in tecnicos:
    ...
    return render_template('rdv.html',
                           ...
                           filtro_data_inicio=filtro_data_ini, # Passa de volta pro HTML
                           filtro_data_fim=filtro_data_fim,    # Passa de volta pro HTML
                           is_admin=is_admin,
                           saldo=saldo_usuario,
                           saldos=saldos,
