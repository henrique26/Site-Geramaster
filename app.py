...
BLING_REDIRECT_URI = os.environ.get('BLING_REDIRECT_URI', 'https://7321-200-146-196-93.ngrok-free.app/callback') 
...
cloudinary.config( 
  cloud_name = os.environ.get('CLOUDINARY_CLOUD_NAME', 'dt1nmvf2d'), 
  api_key = os.environ.get('CLOUDINARY_API_KEY', '565796149712317'), 
  api_secret = os.environ.get('CLOUDINARY_API_SECRET', 'AGC5kBV7S90pY_CGmghncgdjEO0'),
  secure = True
)
...
min_conn = 1
max_conn = 20 # Aguenta até 20 técnicos clicando ao mesmo tempo!

try:
    db_url = os.environ.get('DATABASE_URL', "postgresql://geramaster_db_user:TJ30dPQcC78FISPHI0QLRotCOeskemab@dpg-d0lrj9umcj7s73891d70-a.oregon-postgres.render.com:5432/geramaster_db")
    db_pool = psycopg2.pool.ThreadedConnectionPool(min_conn, max_conn, db_url)
    print("✅ Piscina de conexões (Connection Pool) criada com sucesso!")
except Exception as e:
    print(f"❌ Erro ao criar o pool de conexões: {e}")

# Gerenciador de contexto inteligente (Mágica para o "with get_connection() as conn:" funcionar)
from contextlib import contextmanager

@contextmanager
def get_connection():
    """Pega uma conexão emprestada da piscina e devolve quando terminar."""
...
def exportar_rdv_pdf():
    ...
    # =========================================================
    # --- INÍCIO DA LÓGICA DE NÚMERO MANUAL (NOVO) ---
    # =========================================================
    
    # 1. Pega o número que o usuário digitou na tela
    numero_relatorio_str = request.args.get('numero_relatorio', '').strip()
    
    try:
        # Tenta formatar com 3 zeros à esquerda (ex: se digitar "5", vira "005")
        pdf_report_number_str = f"{int(numero_relatorio_str):03d}"
    except (ValueError, TypeError):
        # Se a pessoa deixou vazio ou digitou letras, usa o que digitou ou "000"
        pdf_report_number_str = numero_relatorio_str if numero_relatorio_str else "000"

    # Define o nome que vai no arquivo (Técnico ou Global)
    if filtro_tecnico_username:
        nome_para_exibicao_no_nome_arquivo = filtro_tecnico_username
    else:
        nome_para_exibicao_no_nome_arquivo = "GLOBAL"

    # 2. Pega o mês e o ano atuais para compor o número
    current_month_str = datetime.now().strftime('%m')
    current_year_str = datetime.now().strftime('%Y')

    # 3. Formata o número do relatório para exibição no PDF (ex: "015/08-2025")
    pdf_number_display = f"{pdf_report_number_str}/{current_month_str}-{current_year_str}"
    
    # Para o nome do arquivo que será baixado (ex: "RDV_henrique_015_08-2025.pdf")
    report_filename_number_part = f"{nome_para_exibicao_no_nome_arquivo}_{pdf_report_number_str}_{current_month_str}-{current_year_str}"
    
    print(f"DEBUG: Número de PDF gerado manualmente: {pdf_number_display}")
    # =========================================================
    # --- FIM DA LÓGICA DE NÚMERO MANUAL ---
    # =========================================================
...
from flask import jsonify # Certifique-se de que o jsonify está importado lá no topo do seu arquivo

@app.route('/os/iniciar/<int:os_id>', methods=['GET', 'POST'])
def iniciar_os(os_id):
    if 'usuario' not in session:
        # Se for requisição offline (POST), devolve erro JSON. Se for navegação normal, redireciona.
        if request.method == 'POST': return jsonify({"erro": "Não autenticado"}), 401
        return redirect(url_for('login'))
    
    try:
        data_hora_inicio = None
        
        # 1. Checa se o pedido veio do nosso Cofre Offline (via POST) e pega o carimbo de tempo
        if request.method == 'POST':
            carimbo_offline = request.form.get('data_hora_offline')
            if carimbo_offline:
                data_hora_inicio = carimbo_offline

        # 2. Salva no Banco de Dados
        with get_connection() as conn:
            with conn.cursor() as cursor:
                if data_hora_inicio:
                    # OFFINE: Usa a hora exata que o celular carimbou
                    cursor.execute("UPDATE os SET data_hora_inicio = %s WHERE id_os = %s", (data_hora_inicio, os_id))
                else:
                    # ONLINE: Uso normal (Pega a hora atual do servidor)
                    cursor.execute("UPDATE os SET data_hora_inicio = NOW() WHERE id_os = %s", (os_id,))
            conn.commit()

        # 3. Responde pro Navegador
        if request.method == 'POST':
            # Resposta silenciosa pro nosso JS do cofre saber que deu tudo certo
            return jsonify({"status": "sucesso"}), 200

        # Resposta normal se ele clicou com internet
        flash('Serviço iniciado com sucesso!', 'success')
        return redirect(url_for('os_executar', os_id=os_id))

    except Exception as e:
        print(f"ERRO AO INICIAR OS: {e}")
        if request.method == 'POST':
            return jsonify({"erro": str(e)}), 500
            
        flash(f'Erro ao iniciar serviço: {e}', 'danger')
        return redirect(url_for('os_executar', os_id=os_id))
...
def processamento_pesado_os(os_id, form_data, arquivos_extraidos):
    print(f"\n[BACKGROUND] Iniciando uploads e IA para OS #{os_id}...")
    try:
        urls_assinaturas = {}

        # 1. ASSINATURAS (Cloudinary)
        for tipo in ['assinatura_tecnico', 'assinatura_cliente']:
            b64_str = form_data.get(f'{tipo}_base64')
            if b64_str and "," in b64_str:
                try:
                    res = cloudinary.uploader.upload(b64_str, folder="geramaster_assinaturas")
                    urls_assinaturas[tipo] = res['secure_url']
                except Exception as e:
                    print(f"Erro assinatura {tipo}: {e}")
                    urls_assinaturas[tipo] = None

        # 2. BANCO DE DADOS (Atualiza OS e Checklist)
        with get_connection() as conn:
            with conn.cursor() as cursor:
                # Salva os textos e URLs das assinaturas
                cursor.execute("""
                    UPDATE os 
                    SET acoes = %s,              
                        observacoes_finais = %s, 
                        assinatura_tecnico = %s, 
                        assinatura_cliente = %s, 
                        nome_assinatura_tecnico = %s, 
                        nome_assinatura_cliente = %s  
                    WHERE id_os = %s
                """, (
                ...
                # Processa os textos do checklist e as FOTOS
                for key, value in form_data.items():
                    if key.startswith('resposta_'):
                        ...
                        if res_eq:
                            ...
                            # Upload de Fotos do Cloudinary em Segundo Plano
                            key_foto = f"foto_{id_vinculo_equip}_{slug_pergunta}"
                            fotos_deste_item = [f for f in arquivos_extraidos if f['key'] == key_foto]
                            
                            for f in fotos_deste_item:
                                try:
                                    file_obj = io.BytesIO(f['bytes']) # Converte os bytes para o Cloudinary ler
                                    res_foto = cloudinary.uploader.upload(file_obj, folder="geramaster_evidencias")
                                    cursor.execute("INSERT INTO checklist_fotos (id_resposta, url_foto) VALUES (%s, %s)", (id_resposta, res_foto['secure_url']))
                                except Exception as e:
                                    print(f"Erro foto {key_foto}: {e}")

                            cursor.execute("UPDATE os_equipamentos SET checklist_realizado = TRUE WHERE id = %s", (id_vinculo_equip,))
            conn.commit()

        # 3. BLING / IA
        texto_pendencias = form_data.get('texto_pendencias', '').strip()
        if texto_pendencias:
            with get_connection() as conn_busca:
                with conn_busca.cursor() as cursor_busca:
                    cursor_busca.execute("SELECT c.nome_cliente FROM os JOIN clientes c ON os.id_cliente = c.id WHERE os.id_os = %s", (os_id,))
                    res_cliente = cursor_busca.fetchone()
                    nome_cliente = res_cliente[0] if res_cliente else "Cliente Não Informado"

            processar_proposta_bling_somente_sku(texto_pendencias, nome_cliente, os_id)
        
        print(f"\n[BACKGROUND] ✅ Sucesso! Tudo processado para a OS #{os_id}")
    except Exception as e:
        print(f"\n[BACKGROUND] ❌ Erro fatal na OS {os_id}: {e}")
...
def finalizar_os(os_id):
    ...
    try:
        # 1. Empacota os textos do formulário
        form_data = dict(request.form)

        # 2. Empacota as fotos direto para a memória RAM (Rápido)
        arquivos_extraidos = []
            ...
            for file in request.files.getlist(key):
                if file and file.filename != '':
                    arquivos_extraidos.append({
                        ...
                        'bytes': file.read() # Puxa o arquivo
                    })

        # 3. Muda o Status da OS para o usuário ver que deu certo na hora
        with get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("UPDATE os SET status = 'Concluída', data_hora_fechamento = NOW() WHERE id_os = %s", (os_id,))
            conn.commit()

        # 4. Dispara a Thread Pesada e LIBERA A TELA NA HORA!
        thread_pesada = threading.Thread(
            target=processamento_pesado_os,
            args=(os_id, form_data, arquivos_extraidos)
        )
        thread_pesada.start()

        flash('OS Finalizada! As fotos e o orçamento estão sendo processados em segundo plano.', 'success')
        return redirect(url_for('os_detalhes', os_id=os_id))

    except Exception as e:
        print(f"ERRO CRÍTICO FINALIZAR: {e}")
        flash(f'Erro ao iniciar finalização: {e}', 'danger')
        return redirect(url_for('os_executar', os_id=os_id))
...
genai.configure(api_key=os.environ.get('GEMINI_API_KEY', 'AIzaSyAbobh287Cx-te8ybLYeZYCY_x0a4ROmm8'))
...
def extrair_pecas_com_ia(texto_pendencias):
    if not texto_pendencias or not texto_pendencias.strip():
        return []

    model = genai.GenerativeModel('gemini-2.5-flash')
    
    prompt = f"""
    Você é um assistente de orçamentos para manutenção de geradores e motores.
    O sistema do ERP agora funciona EXCLUSIVAMENTE via busca por Código SKU Exato.
    Leia o relato do técnico abaixo e extraia apenas os SKUs necessários.
    
    REGRAS DE EXTRAÇÃO:
    1. CÓDIGOS DIRETOS: Se o técnico digitar um código claro (ex: R120-10M, LF777), o 'sku' será exatamente ele.
    2. KITS DE REVISÃO/ÓLEO: Se o técnico pedir um serviço (ex: "Troca de óleo do motor SCANIA DC13"), crie o SKU unindo o serviço, a marca e o modelo. 
       Use o formato: KIT-OLEO-[MARCA]-[MODELO]. (Ex: KIT-OLEO-SCANIA-DC13).
    3. KITS DE FLUIDOS/ARREFECIMENTO: Se o técnico pedir troca de água, aditivo ou líquido de arrefecimento, crie também um kit inteligente contendo a marca e o modelo.
       Use o formato: KIT-ARREFECIMENTO-[MARCA]-[MODELO]. (Ex: KIT-ARREFECIMENTO-SCANIA-DC13).
    4. Identifique a quantidade de kits (geralmente é 1).

    Retorne EXATAMENTE como um array JSON puro com as chaves: 'sku' (sempre em maiúsculo), 'quantidade', e 'nome_original' (para backup).
    Exemplo: 
    [
      {{"sku": "KIT-OLEO-SCANIA-DC13", "quantidade": 1, "nome_original": "troca de óleo e filtros do motor Scania DC13"}},
      {{"sku": "KIT-ARREFECIMENTO-SCANIA-DC13", "quantidade": 1, "nome_original": "troca da água e aditivo do motor, total 30 litros de água"}}
    ]

    Texto do Técnico: {texto_pendencias}
    """
...
def processar_proposta_bling_somente_sku(texto_pendencias, nome_cliente, os_id):
    lista_pecas = extrair_pecas_com_ia(texto_pendencias)
    if not lista_pecas:
        print("Nenhuma peça com código identificada pela IA.")
        return False

    obs_proposta = f"Proposta automatizada - OS #{os_id}.\n"
    obs_proposta += f"\n📋 RELATO DO TÉCNICO:\n\"{texto_pendencias}\"\n"
    
    # 1. Busca Cliente
    contato_payload = {"nome": "Cliente Temporário - Geramaster"} 
    busca_cliente = bling_api_call('contatos', method='GET', data={'pesquisa': nome_cliente})
    if busca_cliente and 'data' in busca_cliente and len(busca_cliente['data']) > 0:
        contato_payload = {"id": busca_cliente['data'][0]['id']}
    else:
        obs_proposta += f"\nATENÇÃO: Cliente '{nome_cliente}' não localizado.\n"

    # 2. Busca Rápida de Produtos (SÓ POR SKU)
    itens_proposta = []
    pecas_nao_encontradas = []

    print("\n--- INICIANDO BUSCA TURBO (SOMENTE SKU) ---")
    for item in lista_pecas:
        sku = item.get('sku', '').upper()
        quantidade = item.get('quantidade', 1)
        nome_original = item.get('nome_original', sku)
        ...
        # Faz uma ÚNICA chamada ao Bling para cada item (velocidade da luz)
        busca_produto = bling_api_call('produtos', method='GET', data={'codigo': sku, 'situacao': 'A'})
        
        if busca_produto and 'data' in busca_produto and len(busca_produto['data']) > 0:
            prod = busca_produto['data'][0]
            itens_proposta.append({
                "produto": {"id": prod['id']},
                "quantidade": quantidade,
                "valor": prod['preco']
            })
            print(f"   ✅ ACHOU: {prod['nome']}")
        else:
            # Se não achou, vai pra lista negra nas observações
            pecas_nao_encontradas.append(f"- {quantidade}x {nome_original} (SKU que a IA procurou: {sku})")
            print(f"   ❌ FALHOU: O código '{sku}' não existe no Bling.")

    # 3. Finaliza a Proposta
    if pecas_nao_encontradas:
        obs_proposta += "\n⚠️ ITENS NÃO LOCALIZADOS (Verifique o Código ou Cadastre o Kit no Bling):\n"
        obs_proposta += "\n".join(pecas_nao_encontradas)

    if len(itens_proposta) > 0:
        payload = {
            "contato": contato_payload,
            "itens": itens_proposta,
            "observacaoInterna": obs_proposta
        }
        res = bling_api_call('propostas-comerciais', method='POST', data=payload)
        
        if res and 'data' in res:
            print("\n🚀 PROPOSTA CRIADA COM SUCESSO!")
            return True
        else:
            print(f"Erro ao salvar a proposta no Bling: {res}")
            return False
    else:
        print("\nProposta abortada: Nenhum dos SKUs lidos existe no estoque.")
        return False   
